...
Deno.serve(
  handler(async (req: Request): Promise<Response> => {
    ...
    //    constraint message.
    // Only a still-LIVE pending request blocks a new one. A request that's past
    // its expires_at but not yet swept by the 60s cron is effectively dead — it
    // must not block a retry after the male already saw "No Response".
